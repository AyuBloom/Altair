...
class PlayerModel extends _CharacterModel {
  ...
  update(dt, user) {
    ...
    if (tick) {
      ...
      if (
        (tick.weaponName && tick.weaponName !== this.lastWeaponName) ||
        (tick.weaponTier && tick.weaponTier !== this.lastWeaponTier)
      ) {
  ...
  updateWeapon(tick, entity) {
    ...
    switch (tick.weaponName) {
      case "Pickaxe":
        var pickaxe = new _SpriteEntity(
          "/asset/image/entity/player/player-pickaxe-t" +
            tick.weaponTier +
            ".svg",
        );
        ...
        break;
      case "Spear":
        var spear = new _SpriteEntity(
          "/asset/image/entity/player/player-spear-t" +
            tick.weaponTier +
            ".svg",
        );
        ...
        break;
      case "Bow":
        var bow = new _SpriteEntity(
          "/asset/image/entity/player/player-bow-t" + tick.weaponTier + ".svg",
        );
        var bowHands = new _SpriteEntity(
          "/asset/image/entity/player/player-bow-t" +
            tick.weaponTier +
            "-hands.svg",
        );
        ...
        break;
      case "Bomb":
        var bomb = new _SpriteEntity(
          "/asset/image/entity/player/player-bomb-t" + tick.weaponTier + ".svg",
        );
        var bombHands = new _SpriteEntity(
          "/asset/image/entity/player/player-bomb-hands.svg",
        );
  ...
  updateHat(tick, entity) {
    ...
    switch (tick.hatName) {
      case "HatHorns":
        var hatHorns = new _SpriteEntity(
          "/asset/image/entity/hat-horns/hat-horns-base.svg",
        );
        this.hat = hatHorns;
        break;
      default:
  ...
  updateHatRotation(tick, networkEntity) {
    var aimingYaw = _Util.interpolateYaw(
  ...
  updateSwingingWeapon(swingLength = 300, swingAmplitude = 100) {
    return (tick, networkEntity) => {
      ...
      if (
        ...
        this.weapon.setRotation(
          aimingYaw - tick.interpolatedYaw - swingDeltaRotation,
        );
        if (this.hat) {
          this.hat.setRotation(
            aimingYaw - tick.interpolatedYaw - swingDeltaRotation * 0.6,
          );
        }
      }
    };
