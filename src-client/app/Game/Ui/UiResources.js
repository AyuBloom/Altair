...
class UiResources extends _UiComponent {
  ...
  onPlayerTickUpdate(playerTick) {
    if (playerTick.wood !== this.lastPlayerTick.wood) {
      this.woodElem.innerHTML = numberAbbreviate(Math.floor(playerTick.wood), 1)
        .toString()
        .toUpperCase();
    }
    if (playerTick.stone !== this.lastPlayerTick.stone) {
      this.stoneElem.innerHTML = numberAbbreviate(
        Math.floor(playerTick.stone),
        1,
      )
        .toString()
        .toUpperCase();
    ...
    if (playerTick.gold !== this.lastPlayerTick.gold) {
      this.goldElem.innerHTML = numberAbbreviate(Math.floor(playerTick.gold), 1)
        .toString()
        .toUpperCase();
    }
    if (playerTick.token !== this.lastPlayerTick.token) {
      this.tokensElem.innerHTML = numberAbbreviate(
        Math.floor(playerTick.token),
        1,
      )
        .toString()
        .toUpperCase();
