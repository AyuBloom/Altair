...
class UiToolbar extends _UiComponent {
  constructor(ui) {
    super(
      ui,
      '<div id="hud-toolbar" class="hud-toolbar">\n            <div class="hud-toolbar-inventory"></div>\n            <div class="hud-toolbar-buildings"></div>\n        </div>',
    );
    this.toolbarInventory = {};
    this.toolbarBuildings = {};
    this.inventoryElem = this.componentElem.querySelector(
      ".hud-toolbar-inventory",
    );
    this.buildingsElem = this.componentElem.querySelector(
      ".hud-toolbar-buildings",
    );
    ...
    for (var itemId in itemSchema) {
      if (itemSchema[itemId].onToolbar) {
        ...
        this.toolbarInventory[itemId].on(
          "equipOrUseItem",
          this.onTriggerEquipOrUseItem.bind(this),
        );
        this.inventoryElem.appendChild(
          this.toolbarInventory[itemId].getComponentElem(),
        );
      }
    }
    for (var buildingId in buildingSchema) {
      this.toolbarBuildings[buildingId] = new _UiToolbarBuilding(
        this.ui,
        buildingId,
      );
      this.toolbarBuildings[buildingId].on(
        "startPlacingBuilding",
        this.onStartPlacingBuilding.bind(this),
      );
      this.toolbarBuildings[buildingId].on(
        "placeBuilding",
        this.onPlaceBuilding.bind(this),
      );
      this.buildingsElem.appendChild(
        this.toolbarBuildings[buildingId].getComponentElem(),
      );
    }
  }
