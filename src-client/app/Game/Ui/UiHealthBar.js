...
class UiHealthBar extends _UiComponent {
  ...
  onPlayerTickUpdate(playerTick) {
    if (
      playerTick.health !== this.lastPlayerTick.health ||
      playerTick.maxHealth !== this.lastPlayerTick.maxHealth
    ) {
      var healthPercentage = Math.round(
        (playerTick.health / playerTick.maxHealth) * 100,
      );
