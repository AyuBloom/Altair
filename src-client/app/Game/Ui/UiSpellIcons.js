...
class UiSpellIcons extends _UiComponent {
    constructor(ui) {
        ...
        function addIconElem(i) {
            ...
            new _UiTooltip(This.iconElems[type], function (elem) {
        ...
        _Game.currentGame.network.addRpcHandler("CastSpellResponse", this.onCastSpellResponse.bind(this));
    }
    onMouseDown(event) {
    ...
    onCastSpellResponse(response) {
        var startTimestamp = performance.now() - Math.max(0, _Game.currentGame.world.getReplicator().getMsSinceTick(response.cooldownStartTick));
