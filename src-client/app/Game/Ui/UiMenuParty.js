...
class UiMenuParty extends _UiComponent {
  constructor(ui) {
    ...
    this.gridJoiningElem =
      this.componentElem.querySelector(".hud-party-joining");
    ...
    this.visibilityElem = this.componentElem.querySelector(
      ".hud-party-visibility",
    );
    ...
    this.visibilityElem.addEventListener(
      "click",
      this.onVisibilityToggle.bind(this),
    );
    ...
    _Game.currentGame.network.addRpcHandler(
      "PartyApplicant",
      this.onPartyApplicant.bind(this),
    );
    _Game.currentGame.network.addRpcHandler(
      "PartyApplicantDenied",
      this.onPartyApplicantDenied.bind(this),
    );
    _Game.currentGame.network.addRpcHandler(
      "PartyApplicantExpired",
      this.onPartyApplicantExpired.bind(this),
    );
  }
  update() {
    ...
    for (var partyId in parties) {
      ...
      if (!this.partyElems[partyId]) {
        ...
        partyElem.addEventListener(
          "click",
          this.onPartyJoinRequestHandler(partyData.partyId).bind(this),
        );
      }
      if (partyData.isOpen) {
    ...
    for (var i in playerPartyMembers) {
      ...
      var memberElem = this.ui.createElement(
        '<div class="hud-member-link">\n                <strong>' +
          ...
          '</small>\n                <div class="hud-member-actions">\n                    <a class="hud-member-can-sell btn' +
          (playerIsLeader && playerPartyMembers[i].isLeader !== 1
            ? ""
            : " is-disabled") +
          (playerPartyMembers[i].canSell === 1 ? " is-active" : "") +
          '"><span class="hud-can-sell-tick"></span> Can sell buildings</a>\n                    <a class="hud-member-kick btn btn-red' +
          (playerIsLeader && playerPartyMembers[i].isLeader !== 1
            ? ""
            : " is-disabled") +
          '">Kick</a>\n                </div>\n            </div>',
      ...
      if (playerIsLeader && playerPartyMembers[i].isLeader === 0) {
        ...
        kickElem.addEventListener(
          "click",
          this.onPartyMemberKick(i).bind(this),
        );
        canSellElem.addEventListener(
          "click",
          this.onPartyMemberCanSellToggle(i).bind(this),
        );
      }
    }
    ...
    this.shareInputElem.value =
      ...
      serverId +
      "/" +
      this.ui.getPlayerPartyShareKey();
    if (playerIsLeader) {
  ...
  onPartyMemberKick(i) {
    ...
    return function (event) {
      ...
      event.stopPropagation();
      popupOverlay.showConfirmation(
        "Are you sure you want to kick this player from your party?",
        10000,
        function () {
          _Game.currentGame.network.sendRpc({
            name: "KickParty",
  ...
  onPartyJoinRequestHandler(partyId) {
    ...
    return function (event) {
      ...
      event.stopPropagation();
      if (
        !linkElem.classList.contains("is-disabled") &&
        !linkElem.classList.contains("is-active")
      ) {
        ...
        var popupOverlay = This.ui.getComponent("PopupOverlay");
        popupOverlay.showConfirmation(
          "Your existing base will be destroyed if you join this party. Are you sure?",
          10000,
          function () {
            This.gridElem.classList.add("is-disabled");
            This.gridJoiningElem.style.display = "block";
  ...
  onPartyApplicant(response) {
    ...
    popupOverlay.showConfirmation(
      "<strong>" + playerName + "</strong> wants to join your party...",
      30000,
      function () {
        _Game.currentGame.network.sendRpc({
