...
class UiShieldBar extends _UiComponent {
  ...
  onPlayerTickUpdate(playerTick) {
    if (
      playerTick.zombieShieldMaxHealth === null ||
      playerTick.zombieShieldMaxHealth === 0
    ) {
      this.hide();
    ...
    if (
      playerTick.zombieShieldHealth !==
        this.lastPlayerTick.zombieShieldHealth ||
      playerTick.zombieShieldMaxHealth !==
        this.lastPlayerTick.zombieShieldMaxHealth
    ) {
      var shieldPercentage = Math.round(
        (playerTick.zombieShieldHealth / playerTick.zombieShieldMaxHealth) *
          100,
      );
