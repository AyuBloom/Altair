...
class UiLeaderboard extends _UiComponent {
  constructor(ui) {
    ...
    this.playersElem = this.componentElem.querySelector(
      ".hud-leaderboard-players",
    );
    _Game.currentGame.network.addRpcHandler(
      "Leaderboard",
      this.onLeaderboardData.bind(this),
    );
  }
  update() {
    ...
    for (var i = 0; i < this.leaderboardData.length; i++) {
      ...
      if (!(i in this.playerElems)) {
        this.playerElems[i] = this.ui.createElement(
          '<div class="hud-leaderboard-player"></div>',
        );
        this.playerRankElems[i] = this.ui.createElement(
          '<span class="player-rank">-</span>',
        );
        this.playerNameElems[i] = this.ui.createElement(
          '<strong class="player-name">-</strong>',
        );
        this.playerScoreElems[i] = this.ui.createElement(
          '<span class="player-score">-</span>',
        );
        this.playerWaveElems[i] = this.ui.createElement(
          '<span class="player-wave">-</span>',
        );
      ...
      this.playerWaveElems[i].innerHTML =
        player.wave === 0
          ? "<small>&mdash;</small>"
          : player.wave.toLocaleString();
    }
    if (this.leaderboardData.length < this.playerElems.length) {
      for (
        var i = this.leaderboardData.length - 1;
        i < this.playerElems.length;
        i++
      ) {
