...
var debug = Debugger("Engine:Network/Replication");
class Replication {
    constructor() {
    ...
    init() {
        ...
        _Game.currentGame.network.addEntityUpdateHandler(this.onEntityUpdate.bind(this));
    ...
    setLatestTickUpdatedCallback(latestTickUpdatedCallback) {
        this.latestTickUpdatedCallback = latestTickUpdatedCallback;
    }
    getClientTimeResets() {
    ...
    onTick(msElapsed) {
        ...
        var steps = 0;
        for (var timestep = 1000 / 60; this.msElapsed >= timestep;) {
        ...
        if (this.currentTick != null) {
            ...
            if (this.shiftedGameTime >= nextTickStart) {
                if (this.interpolating) {
                    ...
                    debug("Extrapolation incident beginning");
                }
    ...
    onEnterWorld(data) {
        if (data.allowed) {
            debug("Effective tick rate: " + data.effectiveTickRate);
            ...
            this.startShiftedGameTime = 0;
            this.interpolating = false;
            _Game.currentGame.ui.components.MenuShop.checkSocialLinks();
            _Game.currentGame.network.sendRpc({ name: "EquipItem", itemName: "PetCARL", tier: 1 });
            _Game.currentGame.network.sendRpc({ name: "EquipItem", itemName: "PetMiner", tier: 1 });
        }
    }
    checkRendererPaused() {
    ...
    onEntityUpdate(data) {
        ...
        if (this.receivedFirstTick) {
            ...
            var differenceInClientTime = data.tick * this.msPerTick - 90 - this.shiftedGameTime;
            if (!rendererPaused) {
                this.differenceInClientTime = differenceInClientTime;
            }
            if (Math.abs(differenceInClientTime) >= 40) {
                this.ticksDesynced2++;
            } else {
                this.ticksDesynced2 = 0;
            }
            if (this.ticksDesynced2 >= 10 || this.wasRendererJustUnpaused) {
                debug("Resetting client time");
                debug("Difference in client time: %f (%f -> %f)", differenceInClientTime, this.shiftedGameTime, data.tick * this.msPerTick - 90);
                debug("Renderer paused: %s", rendererPaused ? "true" : "false");
                debug("Renderer just unpaused: %s", this.wasRendererJustUnpaused ? "true" : "false");
