...
class Replication {
  ...
  init() {
    ...
    _Game.currentGame.network.addEntityUpdateHandler(
    ...
    if (this.currentTick != null) {
      ...
      if (this.shiftedGameTime >= nextTickStart) {
        ...
        this.maxExtrapolationTime = Math.max(
          this.shiftedGameTime - nextTickStart,
          this.maxExtrapolationTime,
        );
        var extrapolationTime = Math.min(
          this.msInThisTick - this.msPerTick,
          this.lastMsElapsed,
        );
  ...
  onEntityUpdate(data) {
    ...
    if (this.receivedFirstTick) {
      ...
      var differenceInClientTime =
        data.tick * this.msPerTick - 90 - this.shiftedGameTime;
      if (!rendererPaused) {
      ...
      if (this.ticksDesynced2 >= 10 || this.wasRendererJustUnpaused) {
        ...
        debug(
          "Difference in client time: %f (%f -> %f)",
          differenceInClientTime,
          this.shiftedGameTime,
          data.tick * this.msPerTick - 90,
        );
        debug("Renderer paused: %s", rendererPaused ? "true" : "false");
        debug(
          "Renderer just unpaused: %s",
          this.wasRendererJustUnpaused ? "true" : "false",
        );
