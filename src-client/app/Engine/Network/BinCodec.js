...
class BinCodec {
  ...
  decodeEntityUpdate(buffer) {
    ...
    while (buffer.remaining()) {
      ...
      var absentEntitiesFlagsLength = Math.floor(
        (this.sortedUidsByType[entityType].length + 7) / 8,
      );
      ...
      for (
        var tableIndex = 0;
        tableIndex < this.sortedUidsByType[entityType].length;
        tableIndex++
      ) {
        var uid = this.sortedUidsByType[entityType][tableIndex];
        if (
          (this.absentEntitiesFlags[Math.floor(tableIndex / 8)] &
            (1 << (tableIndex % 8))) ===
          0
        ) {
          var entity = {
            ...
            if (this.updatedEntityFlags[flagIndex] & (1 << bitIndex)) {
              switch (attribute.type) {
                ...
                case e_AttributeType.Uint64:
                  entity[attribute.name] =
                    buffer.readUint32() + buffer.readUint32() * 4294967296;
                  break;
